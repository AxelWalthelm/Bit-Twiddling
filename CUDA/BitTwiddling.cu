...
{
    int n_rep = 1; // 3    // negative values skip combinations generated by runs with smaller abs(n_rep)
    int n_sel = 8; // 8
    int n_shi = 6; // 4-6
    // 1 8 4: no solution
    // 1 8 5: no solution
    ...
    constexpr char iteration0_file[] = "results_last_item.txt";
    {
        ...
        if (file)
        {
            if (4 != fscanf(file, "%d %d %d %" PRIu64 "\n", &n_rep, &n_sel, &n_shi, &iteration0))
                throw std::runtime_error("failed to read file " + std::string(iteration0_file));
            fclose(file);
        }
    }
    ...
    // iteration0 = div_ceil(uint64_t(35) * select.count * shift.count, iteration_blocks * threads * steps)
