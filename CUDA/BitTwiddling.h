...
void semi_exhaustive_search_for_8bit_rev_cpu(unsigned int gridDim_x, unsigned int blockDim_x, uint64_t start, int steps, int n_rep, int n_sel, int n_shi);

namespace{

strong_inline CUDA_ALL
void semi_exhaustive_search_for_8bit_rev_impl(unsigned int blockIdx_x, unsigned int blockDim_x, unsigned int threadIdx_x, uint64_t start, int steps, int n_rep, int n_sel, int n_shi)
{
...
void semi_exhaustive_search_for_8bit_rev_kernel(uint64_t start, int steps, int n_rep, int n_sel, int n_shi)
{
    ...
    assert(blockDim.y == 1);
    assert(blockDim.z == 1);
    semi_exhaustive_search_for_8bit_rev_impl(blockIdx.x, blockDim.x, threadIdx.x, start, steps, n_rep, n_sel, n_shi);
